...
const MarksUpload = () => {
  ...
  const handleViewMarks = async () => {
    ...
    try {
      const res = await marksAPI.getByStudent(selectedStudentId);
  ...
  return (
    <div>
      ...
      {viewMode === 'upload' && (user?.role === 'ADMIN' || user?.role === 'TEACHER') ? (
        <div>
              ...
              {students.length > 0 && bulkSubject ? (
                <>
                  ...
                  <div style={{ marginTop: '1.5rem', display: 'flex', justifyContent: 'flex-end' }}>
                    <button className="btn-primary" onClick={handleBulkSubmit} style={{ width: 'auto', padding: '0.7rem 2rem' }}>
                      Upload All Marks
                    </button>
                  </div>
                </>
                ...
                <div className="empty-state">
                  <p>{!bulkSubject ? 'Select a subject to begin' : 'No students found'}</p>
                </div>
              )}
            </div>
          )}
